      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-2">
          <input
            ...
            min={filters.priceRange.min}
            max={priceRange[1]} // cannot exceed max handle
            step={100}
            value={priceRange[0]}
            onChange={(e) => {
              const newMin = Number(e.target.value);
              if (newMin <= priceRange[1]) {
                onFilterChange("priceRange", [newMin, priceRange[1]]);
              }
            }}
          />
          ...
          <input
            type="number"
            className="w-full border rounded px-2 py-1 text-sm"
            min={priceRange[0]} // cannot be less than min handle
            max={filters.priceRange.max}
            step={100}
            value={priceRange[1]}
            onChange={(e) => {
              ...
              if (newMax >= priceRange[0]) {
                onFilterChange("priceRange", [priceRange[0], newMax]);
              }
            }}
          />
        </div>
