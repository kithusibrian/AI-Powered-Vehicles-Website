...
export async function processCarImageWithAI(file) {
  try {
    //Check if Api Key is properly configured and is available

    if (!process.env.GEMINI_API_KEY) {
      throw new Error(" We Mzee API key is not configured properly");
    }
    ...
    //The most important thing which is writing the prompt for the car details extraction
    ...
    //Parse the JSON Respinse from Gemini
...
export async function getCars(search = "") {
  try {
    ...
    //These are the search terms that when the user enters will return results More will be added to this to smoothen user experience
    if (search) {
      where.OR = [
